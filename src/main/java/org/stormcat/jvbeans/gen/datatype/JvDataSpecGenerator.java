...
public class JvDataSpecGenerator extends BaseGenerator {
	...
	public JvDataSpecGenerator(String packageName, ItemType itemType,
			List<DataTypeRow> list) {
		super(packageName);
	...
	protected void prepare(VelocityContainer container) {
		...
		container.addParameter("dataName",
				list.get(0).getDataName().replace("\n", ""));
	...
	private String getDtoType(String recordTypeId) {
		return "org.stormcat.jvbeans.jvlink.definitions.dto."
				+ PropertyUtil.getValue(String.format("%s.%s",
						"org.stormcat.jvbeans.gen.format", recordTypeId),
						"dto.name");
	}
